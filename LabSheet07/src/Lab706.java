...
public class Lab706 {
	...
	int prevData = prevData(nums,indexNumb);
	int nextData = nextData(nums,indexNumb);
	JOptionPane.showMessageDialog(null, "Current data, nums["+indexNumb+"] is "+ currentData(nums,indexNumb)+
			"\n"+((indexNumb-1<0)?"no previous data":"Previous data,nums["+(indexNumb-1)+"] is "+ nums[indexNumb-1]+
